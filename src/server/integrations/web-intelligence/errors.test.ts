...
describe("normalizeError", () => {
  ...
  it("allows overriding the error type explicitly", () => {
    ...
    expect(error.message).toContain("integração");
  });

  it("never leaks the vendor name in any error message", () => {
    for (const type of ["configuration", "authentication", "forbidden", "server_error", "network", "timeout"] as const) {
      const error = normalizeError(500, undefined, undefined, type);
      expect(error.message.toLowerCase()).not.toContain("scrapegraphai");
      expect(error.message.toLowerCase()).not.toMatch(/\bsgai\b/);
    }
  });

  it("translates known infrastructure phrases instead of leaking them", () => {
    const error = normalizeError(401, "Unauthorized: invalid credentials for provider api.example-vendor.com");
    expect(error.originalMessage).toBe("A integração não está configurada corretamente.");
  });
});
