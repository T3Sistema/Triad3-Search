      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-text-secondary">
        ...
        {status !== undefined && (
          <span className="inline-flex items-center gap-1">
            Status: <Badge variant={toneForStatus(String(status))}>{translateStatus(String(status))}</Badge>
          </span>
        )}
        {typeof elapsedMs === "number" && <span>Tempo: {formatMilliseconds(elapsedMs)}</span>}
